...
def getDescentImageTinyListfromOriginData(
    ...
    while i < min(endFrame, len(filenames)):
        ...
        #直接进行掩膜
        descentImage.data=descentImage.data*(BinaryMask/255)
        descentImage.data=descentImage.data.astype(np.uint8)
        ...
        last_saved_idx = i
        descent_img_idx += 1
        print('当前共抽出{}帧下降图像'.format(descent_img_idx+1))
        # 为下一次迭代准备
        i += 1
